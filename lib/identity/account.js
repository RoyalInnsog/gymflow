...
// Test-OTP mode: while codes are UNDELIVERABLE (no real OTP_PROVIDER wired up),
// the fixed code 000000 verifies and is surfaced to the UI — otherwise phone
// verification is impossible on a deployed instance. Wiring a real provider
// disables it automatically; ALLOW_TEST_OTP=false force-disables it sooner.
// ponytail: prototype-wide master code; replace with a real provider at go-live.
function testOtpActive() {
  if (process.env.NODE_ENV === 'production') return false;
  if (process.env.ALLOW_TEST_OTP === 'false') return false;
  return process.env.ALLOW_TEST_OTP === 'true' || (process.env.OTP_PROVIDER || 'console') === 'console';
}
...
async function verifyPhoneOtp(userId, phone, codeInput) {
  ...
  const masterOk = testOtpActive() && String(codeInput || '').trim() === '000000';
  if (!masterOk && core.sha256(String(codeInput || '').trim()) !== row.otp_hash) {
...
async function linkProvider(userId, provider, providerUid, email) {
  await runQuery(
    `INSERT INTO identity_providers (id, user_id, provider, provider_uid, email, last_used_at) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT DO NOTHING`,
...
module.exports = {
  ...
  requestPhoneOtp, verifyPhoneOtp, testOtpActive,
