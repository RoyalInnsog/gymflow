...
async function verifyPhoneOtp(userId, phone, codeInput) {
  ...
  // Prototype escape hatch: deliverOtp() has no real SMS/WhatsApp provider yet,
  // so a deployment may set ALLOW_TEST_OTP=true to accept the fixed code 000000.
  // Remove once a provider is wired up — this must never ship enabled at go-live.
  const masterOk = process.env.ALLOW_TEST_OTP === 'true' && String(codeInput || '').trim() === '000000';
