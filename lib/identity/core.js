...
function rateLimit({ windowMs, max, message, refundOnSuccess = true, keyFn = null }) {
  ...
  setInterval(() => {
    const now = Date.now();
    for (const [k, v] of memoryHits) if (now - v.start > windowMs) memoryHits.delete(k);
  }, windowMs).unref();
  ...
  return async (req, res, next) => {
    ...
    if (redisClient && redisClient.status === 'ready') {
      try {
        ...
        if (refundOnSuccess) {
          res.on('finish', () => { 
            if (res.statusCode < 400) redisClient.decr(key).catch(()=>{}); 
          });
        }
        return next();
      } catch (err) {
        // Fallthrough to memory if Redis fails during execution
      }
    }

    // 2. MEMORY PATH (Fallback)
    let rec = memoryHits.get(key);
    if (!rec || now - rec.start > windowMs) { rec = { start: now, count: 0 }; memoryHits.set(key, rec); }
    if (rec.count >= max) {
