...
// ---------------------------------------------------------------------------
// Rate limiting — dependency-free, per IP+route sliding window. In-memory by
// design (single-instance deployment); swap for a shared store when scaling out.
// ---------------------------------------------------------------------------
function rateLimit({ windowMs, max, message, refundOnSuccess = true }) {
  ...
  return (req, res, next) => {
    ...
    rec.count++;
    // Successful attempts are refunded so shared-NAT users are not locked out —
    // but always-200 endpoints (forgot-password, resend, OTP send) must use the
    // non-refunding limiter or they would have no limit at all.
    if (refundOnSuccess) {
...
const revokedSids = new Map();   // sid -> expiry ms
...
function revokeSidLocal(sid) {
  if (sid) revokedSids.set(sid, Date.now() + ACCESS_TTL_MS + 60 * 1000);
}
setInterval(() => {
  const now = Date.now();
  for (const [k, exp] of revokedTokens) if (exp < now) revokedTokens.delete(k);
  for (const [k, exp] of revokedSids) if (exp < now) revokedSids.delete(k);
}, 60 * 60 * 1000).unref();
...
function setAuthCookie(res, token, remember, pending) {
  res.cookie('auth_token', token, { ...COOKIE_BASE, maxAge: pending ? PENDING_TOKEN_TTL_MS : ACCESS_TTL_MS });
}
function setRefreshCookie(res, rawToken, remember) {
  res.cookie('refresh_token', rawToken, { ...COOKIE_BASE, maxAge: remember ? SESSION_REMEMBER_TTL_MS : SESSION_TTL_MS });
}
...
function setDeviceCookie(res, rawToken) {
  res.cookie('device_token', rawToken, { ...COOKIE_BASE, maxAge: 400 * 24 * 60 * 60 * 1000 });
}
...
function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, JWT_SECRET, { algorithms: ['HS256'] }, (err, user) => {
    ...
    if (user && ((user.jti && revokedTokens.has(user.jti)) || (user.sid && revokedSids.has(user.sid)))) {
...
module.exports = {
  ...
  parseUserAgent,
  rateLimit, authLimiter, sensitiveLimiter,
  revokeToken, revokeSidLocal, revokedTokens, revokedSids,
