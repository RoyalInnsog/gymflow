 ...
 *
 * `trial` is a 7-day trial of PRO — same features as pro (WhatsApp allowance
 * capped at TRIAL_WHATSAPP_ALLOWANCE), with subscription_status = 'trial';
 * auto-downgraded to free basic on expiry if unpaid (routes/api.js middleware).
 *
...
// Trials get PRO features but a REDUCED WhatsApp allowance: at Meta India rates
// (~₹0.8/marketing msg) Pro's 200-message allowance would gift ~₹160 of real
// spend to every throwaway trial signup. 50 messages is plenty to demo the
// automations across a 7-day trial without funding spam.
const TRIAL_WHATSAPP_ALLOWANCE = 50;
const isTrialPlan = p => String(p || '').toLowerCase() === 'trial';
...
function getLimits(plan) {
  ...
  return {
    ...
    maxWhatsAppMessages: isTrialPlan(plan) ? TRIAL_WHATSAPP_ALLOWANCE : c.whatsappAllowance,
    topUpCap: c.topUpCap,
    multiGym: c.multiGym,
    price: c.price
  };
...
function whatsappAllowanceFor(plan) {
  return isTrialPlan(plan) ? TRIAL_WHATSAPP_ALLOWANCE : getPlan(plan).whatsappAllowance;
}
