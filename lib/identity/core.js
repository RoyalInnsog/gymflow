...
// Hardcoded to prevent ~100ms startup penalty.
const DUMMY_PW_HASH = '$2a$10$NlOM6Pll.388LtunkD/gkeaYKEThctSRMtn5O6edPI.JQcnk45HUy';
...
// [SEC/TODO] Consider shorter session windows (e.g. 1h / 7d) or implementing
// a sliding token refresh in future iterations to limit hijack windows.
...
//
// [CAPACITOR] The Capacitor native app now uses the Authorization: Bearer
// header (token retrieved from Capacitor Secure Storage) instead of cookies,
// so there is no need to emit cross-site cookies. All cookies remain
// SameSite=Lax for strong CSRF protection on the web app.
// ---------------------------------------------------------------------------
const COOKIE_BASE = { httpOnly: true, sameSite: 'lax', secure: IS_PROD };

// Returns the cookie options for the request. All cookies are SameSite=Lax
// for strong CSRF protection. The Capacitor native app uses Authorization header
// instead of cookies, so no cross-site cookies are needed.
...
// [ROLES] Server-side role spine. Roles are assigned server-side and encoded
// into scoped tokens; client requests can never select their own role.
...
function authorize(...required) {
  return async (req, res, next) => {
    try {
      ...
      return res.status(403).json({ error: 'Permission check failed.', code: 'PERMISSION_DENIED' });
    }
  };
}
